...
To characterize how much the cohort distribution depends on grid
spacing, we ran the full 64 subjects at both \SI{2}{\milli\metre} and
\SI{1}{\milli\metre}, plus a five-subject convergence subset spanning
the dx=1\,mm distribution at \SI{0.5}{\milli\metre}.
...
kable(conv_summary, format = "latex", booktabs = TRUE, row.names = FALSE,
      ...
      caption = sprintf("Cohort-wide grid convergence (Birnbaum, n=64). Refining from 2\\,mm to 1\\,mm raises median focal pressure by %.1f$\\times$ (subject-wise mean ratio %.2f$\\times$, range %.2f--%.2f$\\times$). Spread is preserved (5.9 to 5.5).",
                        median(dx1$p_brain_max_kpa)/median(dx2$p_brain_max_kpa),
                        mean(ratios), min(ratios), max(ratios)),
      escape = FALSE)
@

The non-uniform cohort-wise convergence behavior (subject-wise dx=1/dx=2
ratios from \Sexpr{sprintf("%.2f", min(ratios))}$\times$ to
\Sexpr{sprintf("%.2f", max(ratios))}$\times$) indicates that thinner
or more sparsely sampled skulls under-resolve more strongly at
\SI{2}{\milli\metre}. Combined with the BM1 free-field convergence
(\SI{0.5}{\milli\metre}: 44\% error, \SI{0.25}{\milli\metre}: 12\%),
the \SI{1}{\milli\metre} cohort numbers are themselves a lower bound.
We therefore treat the cohort \emph{spread} (5.5--5.9$\times$) as the
robust per-subject finding and the absolute pressures as
resolution-limited.
