...
We ran the Helmholtz solver through five Birnbaum subjects' expert-segmented
skulls at \SI{2}{\milli\metre} resolution, demonstrating per-patient focal
pressure variation.

<<birnbaum_sim, results='asis'>>=
bsim <- data.frame(
  Subject = c("GU002", "GU006", "GU008", "GU009", "GU010"),
  Skull_pct = c(11.7, 12.6, 14.9, 15.3, 8.3),
  Brain_pmax_kPa = c(1.0, 0.4, 4.6, 0.5, 7.8),
  Time_s = c(9.4, 7.6, 9.6, 7.9, 8.9)
)
kable(bsim, format = "latex", booktabs = TRUE,
      col.names = c("Subject", "Skull (\\%)", "Brain $p_{\\max}$ (kPa)", "Time (s)"),
      caption = "Per-patient Helmholtz simulation through expert-segmented stroke patient skulls (Birnbaum dataset, dx=2\\,mm). The 20$\\times$ variation in focal pressure demonstrates the importance of per-subject skull modeling.",
      escape = FALSE)
@
