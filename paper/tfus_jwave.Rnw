...
project_root <- "/Users/mhough/dev/openlifu-python"
paper_dir    <- file.path(project_root, "paper")
fig_dir      <- file.path(paper_dir, "figures")
bench_dir    <- file.path(project_root, "benchmarks")
@
...
<<tissue_table, results='asis'>>=
tissues <- data.frame(
  Tissue = c("Water", "Scalp", "Skull", "CSF", "Gray Matter", "White Matter"),
  c_ms = c(1500, 1610, 4080, 1500, 1560, 1560),
  rho_kgm3 = c(1000, 1090, 1900, 1000, 1040, 1040),
  alpha_dBcm = c(0.0, 3.5, 4.74, 0.0, 5.3, 5.3)
)
kable(tissues, format = "latex", booktabs = TRUE,
      col.names = c("Tissue", "$c$ (m/s)", "$\\rho$ (kg/m$^3$)",
                     "$\\alpha$ (dB/cm/MHz)"),
      caption = "Acoustic properties for transcranial tissue modeling.",
      escape = FALSE)
@
...
<<bm4_results, results='asis'>>=
# BM4-SC1 results from Modal run and Zenodo reference extraction
bm4 <- data.frame(
  Model = c("This work (Helmholtz)", "j-Wave (ref)", "MSOUND (ref)",
            "OPTIMUS (ref)", "SALVUS (ref)", "SIM4LIFE (ref)"),
  Focus_kPa = c(262.5, 257.7, 270.7, 266.5, 262.1, 269.0),
  Peak_x_mm = c(53.5, 57.0, 58.0, 57.0, 57.0, 56.5),
  Peak_kPa = c(329.8, 299.6, 301.9, 308.3, 305.5, 317.8)
)
kable(bm4, format = "latex", booktabs = TRUE,
      col.names = c("Model", "$p_{\\text{amp}}$ @ focus (kPa)",
                     "Peak $x$ (mm)", "Brain peak (kPa)"),
      caption = "ITRUSST BM4-SC1 results at dx=0.5\\,mm. Focus at geometric focal point (x=64\\,mm). Our Helmholtz solver matches the reference j-Wave within 1.9\\%.",
      escape = FALSE)
@
...
<<solver_comparison, results='asis'>>=
solvers <- data.frame(
  Solver = c("Helmholtz (CW)", "Time-domain (60 cyc)"),
  Focus_kPa = c(262.5, "---"),
  Sim_s = c(28.8, 6.0),
  Memory = c("~200 MB", "~53 GB (projected)"),
  Grid = c("241x141x141", "121x71x71")
)
kable(solvers, format = "latex", booktabs = TRUE,
      col.names = c("Solver", "$p_{\\text{amp}}$ @ focus", "Time (s)",
                     "GPU Memory", "Grid"),
      caption = "Helmholtz vs.\\ time-domain solver comparison on A100 GPU.",
      escape = FALSE)
@
...
<<het_results, results='asis'>>=
het <- data.frame(
  Mode = c("Heterogeneous skull", "Homogeneous water"),
  p_max_kPa = c(1215, 932),
  Sim_s = c(4.4, 4.1),
  Tissues = c(6, 1)
)
kable(het, format = "latex", booktabs = TRUE,
      col.names = c("Mode", "$p_{\\max}$ (kPa)", "Time (s)", "Tissues"),
      caption = "Heterogeneous vs.\\ homogeneous simulation on A100 (time-domain, 61x61x65 grid).",
      escape = FALSE)
@
...
\subsection{BM1: Free-field analytical validation}

We validated the source model against the analytical focal pressure for
a focused bowl in lossless water ($p_{\text{focus}} = \rho c v_0 k a^2 / 2R
\approx \SI{1005}{\kilo\pascal}$).

<<bm1_results, results='asis'>>=
bm1 <- data.frame(
  dx_mm = c(0.5, 0.25),
  Focus_kPa = c(562, 881),
  Analytical_kPa = c(1005, 1005),
  Error_pct = c(44.1, 12.4),
  Time_s = c(24, 455)
)
kable(bm1, format = "latex", booktabs = TRUE,
      col.names = c("dx (mm)", "Focus (kPa)", "Analytical (kPa)", "Error (\\%)", "Time (s)"),
      caption = "BM1 free-field validation: focal pressure converges toward the analytical solution with decreasing grid spacing.",
      escape = FALSE)
@

The error reduction from 44\\% at \SI{0.5}{\milli\metre} to 12\\% at
\SI{0.25}{\milli\metre} confirms grid convergence; the residual error
is attributable to the discrete 256-element point-source approximation
of a continuous bowl surface.
...
\subsection{Per-patient skull simulation}

We ran the Helmholtz solver through all 64 Birnbaum subjects'
expert-segmented skulls at \SI{1}{\milli\metre} resolution
(Table~\ref{tab:birnbaum_sim}). Per-patient focal pressure varied by
5.5$\times$ across the cohort, with absolute values in the 11--60\,kPa
range. Convergence behavior is non-uniform: the same cohort run at
\SI{2}{\milli\metre} resolution yielded a 3.4$\times$ lower mean
(Section~\ref{sec:birnbaum_convergence}), but the cohort spread ratio
was preserved (5.9$\times$ vs.\ 5.5$\times$), indicating that
inter-subject variability is robust to grid spacing while absolute
focal pressures are not.

<<birnbaum_sim, results='asis'>>=
suppressPackageStartupMessages(library(jsonlite))
bsim_path <- file.path(project_root, "results",
                       "birnbaum_n64_dx1.0mm.json")
bsim_json <- fromJSON(bsim_path)
bsim <- bsim_json$results
bsim$p_brain_max_kpa <- bsim$p_brain_max_pa / 1e3
qs <- quantile(bsim$p_brain_max_kpa, c(0, 0.25, 0.5, 0.75, 1.0))
ord <- order(bsim$p_brain_max_kpa)
pick <- bsim[ord[c(1,
                   which.min(abs(bsim$p_brain_max_kpa[ord] - qs[2])),
                   which.min(abs(bsim$p_brain_max_kpa[ord] - qs[3])),
                   which.min(abs(bsim$p_brain_max_kpa[ord] - qs[4])),
                   length(ord))], ]
pick$Quantile <- c("min", "p25", "median", "p75", "max")
pick <- pick[, c("Quantile", "subject", "p_brain_max_kpa", "sim_time_s")]
kable(pick, format = "latex", booktabs = TRUE, row.names = FALSE,
      col.names = c("Quantile", "Subject", "Brain $p_{\\max}$ (kPa)", "Time (s)"),
      digits = c(0, 0, 1, 1),
      caption = sprintf("Per-patient Helmholtz simulation across all %d Birnbaum subjects (dx=1\\,mm, 500\\,kHz, 60\\,kPa source). Representative quantiles shown; full distribution: mean %.1f\\,kPa, range %.1f--%.1f\\,kPa, IQR %.1f--%.1f\\,kPa.",
                        nrow(bsim), mean(bsim$p_brain_max_kpa),
                        qs[1], qs[5], qs[2], qs[4]),
      escape = FALSE)
@

\subsubsection{Grid convergence across the cohort}
\label{sec:birnbaum_convergence}

We ran all 64 subjects at \SI{2}{\milli\metre} and
\SI{1}{\milli\metre} to characterise cohort-wide convergence
behaviour, plus a five-subject subset spanning the dx=1\,mm
distribution at \SI{0.5}{\milli\metre} (constant physical domain via
nearest-neighbour upsampling of the native 1\,mm CHARM-style labels).

<<birnbaum_convergence, results='asis'>>=
dx2 <- fromJSON(file.path(project_root, "results", "birnbaum_64subjects",
                          "dx2mm_partial.json"))$results
dx1 <- fromJSON(file.path(project_root, "results",
                          "birnbaum_n64_dx1.0mm.json"))$results
dx1$p_brain_max_kpa <- dx1$p_brain_max_pa / 1e3

m2 <- setNames(dx2$p_brain_max_kpa, dx2$subject)
m1 <- setNames(dx1$p_brain_max_kpa, dx1$subject)
common <- intersect(names(m1), names(m2))
ratios12 <- m1[common] / m2[common]

conv_summary <- data.frame(
  dx_mm = c(2.0, 1.0),
  median_kpa = c(median(dx2$p_brain_max_kpa), median(dx1$p_brain_max_kpa)),
  mean_kpa = c(mean(dx2$p_brain_max_kpa), mean(dx1$p_brain_max_kpa)),
  range_min = c(min(dx2$p_brain_max_kpa), min(dx1$p_brain_max_kpa)),
  range_max = c(max(dx2$p_brain_max_kpa), max(dx1$p_brain_max_kpa)),
  spread = c(max(dx2$p_brain_max_kpa)/min(dx2$p_brain_max_kpa),
             max(dx1$p_brain_max_kpa)/min(dx1$p_brain_max_kpa))
)
kable(conv_summary, format = "latex", booktabs = TRUE, row.names = FALSE,
      col.names = c("dx (mm)", "Median (kPa)", "Mean (kPa)",
                    "Min (kPa)", "Max (kPa)", "Max/Min"),
      digits = c(2, 1, 1, 1, 1, 2),
      caption = sprintf("Cohort-wide grid convergence (Birnbaum, n=64). Refining from 2\\,mm to 1\\,mm raises median focal pressure by %.1f$\\times$ (subject-wise mean %.2f$\\times$, range %.2f--%.2f$\\times$); cohort spread is preserved (%.1f to %.1f).",
                        median(dx1$p_brain_max_kpa)/median(dx2$p_brain_max_kpa),
                        mean(ratios12), min(ratios12), max(ratios12),
                        max(dx2$p_brain_max_kpa)/min(dx2$p_brain_max_kpa),
                        max(dx1$p_brain_max_kpa)/min(dx1$p_brain_max_kpa)),
      escape = FALSE)
@

\paragraph{Per-subject convergence trajectories.}
We additionally ran a five-subject subset (the dx=1\,mm min, p25,
median, p75, max subjects) at \SI{0.5}{\milli\metre} on an
A100-80GB, with categorical labels nearest-neighbour upsampled to
preserve physical domain extent.

<<birnbaum_subject_traj, results='asis'>>=
sub5 <- fromJSON(file.path(project_root, "results",
    "birnbaum_GU010_NC019_NYU010_NYU009_GU012_dx0.5mm.json"))$results
sub5$p_brain_max_kpa <- sub5$p_brain_max_pa / 1e3
m05 <- setNames(sub5$p_brain_max_kpa, sub5$subject)
sids <- c("GU010", "NC019", "NYU010", "NYU009", "GU012")
traj <- data.frame(
  subject = sids,
  dx2 = round(m2[sids], 1),
  dx1 = round(m1[sids], 1),
  dx05 = round(m05[sids], 1),
  r1_2 = round(m1[sids] / m2[sids], 2),
  r05_1 = round(m05[sids] / m1[sids], 2)
)
kable(traj, format = "latex", booktabs = TRUE, row.names = FALSE,
      col.names = c("Subject", "dx=2", "dx=1", "dx=0.5",
                    "$r_{1/2}$", "$r_{0.5/1}$"),
      caption = "Per-subject focal pressure (kPa) at three grid resolutions for the five-subject convergence subset spanning the dx=1\\,mm distribution. The dx=0.5\\,mm step uses nearest-neighbour categorical upsampling of the 1\\,mm CHARM labels so the physical domain extent is held constant. GU012 (thick skull) shows the strongest sign of asymptotic convergence; GU010 (thin skull) is the slowest to settle, plausibly reflecting the discrete-bowl source's under-resolution of the focal spot when skull attenuation is weak.",
      escape = FALSE)
@

Combined with the BM1 free-field convergence trend
(\SI{0.5}{\milli\metre}: 44\% error, \SI{0.25}{\milli\metre}: 12\%
versus the analytic baseline), the dx=\SI{0.5}{\milli\metre} cohort
numbers in Table~\ref{tab:birnbaum_subject_traj} are themselves a
lower bound on the converged absolutes. We therefore report the
cohort \emph{spread} as the robust per-subject finding (5.5--5.9$\times$
preserved across resolutions) and the absolute focal pressures as
grid-limited at the chosen working resolution. Pushing to
\SI{0.25}{\milli\metre} on the upsampled cohort exceeded
A100-80GB memory at the full-head domain in our test runs; cropping
the simulation domain to a tight skull bounding box would make
the converged-absolute experiment tractable and is the natural next
step.

\subsection{Birnbaum cohort: 64-subject skull analysis}

We validated the tissue segmentation and pseudo-CT pipeline across the
full Birnbaum dataset~\citep{Birnbaum2025}: 64 subjects with
expert-corrected 7-class head segmentations from T1-weighted MRI at
\SI{1}{\milli\metre} isotropic resolution, spanning four clinical
sites (Georgetown, UNC, NYU, healthy controls).

<<birnbaum_cohort, results='asis'>>=
bn <- read.csv(file.path(project_root, "paper", "birnbaum_results.csv"))
# Aggregate by cohort
agg <- aggregate(cbind(n_skull, thickness_mean_mm, pseudo_ct_bone_dice) ~ cohort, bn,
                 function(x) paste0(round(mean(x),1), " +/- ", round(sd(x),1)))
n_per <- table(bn$cohort)
agg$n <- as.integer(n_per[agg$cohort])
agg <- agg[, c("cohort", "n", "n_skull", "thickness_mean_mm", "pseudo_ct_bone_dice")]
kable(agg, format = "latex", booktabs = TRUE,
      col.names = c("Cohort", "N", "Skull vol (voxels)", "Thickness (mm)", "Pseudo-CT Dice"),
      caption = "Birnbaum cohort skull analysis: 64 subjects across 4 clinical sites. Pseudo-CT Dice (mean $\\pm$ SD) confirms the limitation of threshold-based T1-only skull segmentation.",
      escape = FALSE)
@

Cross-site variation in skull thickness (\SIrange{22}{27}{\milli\metre})
and volume demonstrates why per-subject tissue modeling is essential for
accurate tFUS treatment planning. The mean pseudo-CT bone Dice of
$0.203 \pm 0.059$ quantifies the gap between simple threshold
segmentation and expert annotation, motivating learned approaches such
as nnU-Net~\citep{Isensee2021nnunet} (0.862 Dice reported on this
dataset in PR~\#436).

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/fig6_birnbaum_cohort.png}
  \caption{Birnbaum cohort: skull volume and thickness distributions by
    clinical site, pseudo-CT bone Dice by cohort, and skull vs.\ brain
    volume relationship (64 subjects, 4 sites).}
  \label{fig:birnbaum}
\end{figure}
