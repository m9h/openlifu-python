...
We ran the Helmholtz solver through all 64 Birnbaum subjects' expert-segmented
skulls at \SI{2}{\milli\metre} resolution. Per-patient focal pressure
varied by a factor of 5.9$\times$ across the cohort (Table~\ref{tab:birnbaum_sim}).

<<birnbaum_sim, results='asis'>>=
suppressPackageStartupMessages(library(jsonlite))
bsim_path <- file.path(project_root, "results", "birnbaum_64subjects",
                       "dx2mm_partial.json")
...
kable(pick, format = "latex", booktabs = TRUE, row.names = FALSE,
      ...
      digits = c(0, 0, 1, 1),
      caption = sprintf("Per-patient Helmholtz simulation across all %d Birnbaum subjects (dx=2\\,mm, 500\\,kHz, 60\\,kPa source). Representative quantiles shown; full distribution: mean %.1f\\,kPa, range %.1f--%.1f\\,kPa, IQR %.1f--%.1f\\,kPa.",
